...
import { monthlyQues } from "@/Constants/monthlyFormQues";
...
function Monthly() {
  ...
  const handleSubmit = async (e: any) => {
   ...
   try {
     setSubmitting(true)
     const result = await axios.post("/api/monthlyForm", formData);
     setSubmitting(false)
     setSubmitted(true)
   } catch (error) {
    setSubmitting(false)
      ...
      <div className="w-[45%]">
        ...
        <div className="bg-white rounded-lg mt-4 p-4 border-0 border-t-8 border-red-600">
          <div className="w-full text-left">
            <h1 className="text-[32px] font-medium">Moguls Monthly Check</h1>
        ...
        <form onSubmit={handleSubmit}>
          {monthlyQues.map((item:any, index:any) => (
